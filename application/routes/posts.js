...
const storage = multer.diskStorage({
    ...
    filename: function (req, file, cb) {
     ...
     let fileExt = file.mimetype.split("/")[1];
      cb(null, `${file.filename}-${Date.now()}-${Math.round(Math.random()* 1e9)}.${fileExt}}`)
    }
  })
...
router.post("/create", upload.single("uploadImage"), function(req,res,next){
    ...
    sharp(uploadedFile).resize(200).toFile(destinationOfThumbnail).then(function() {
        let baseSQL = ` INSERT INTO posts (title, description, image, thumbnail, fk_authorId) 
        VALUE (?, ?, ?, ?, ?)`
       return db.query(baseSQL, [title, description, uploadedFile, destinationOfThumbnail
        , userId])
    })
